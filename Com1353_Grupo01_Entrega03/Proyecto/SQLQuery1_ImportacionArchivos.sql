...
BEGIN
    ...
    PRINT 'SP Importar_ElectronicAccessories ya existe --> se borrÛ';
END;
GO
...
BEGIN
    ...
    -- Crear tabla temporal para cargar los datos(Para que sea accesible en toda la ejecuciÛn del procedimiento, usa una tabla temporal global (##Temp))
CREATE TABLE ##Temp (
...
PRINT 'El valor de compra del dÛlar es: ' + CAST(@precioCompra AS VARCHAR);



-- Ahora la tabla ya es accesible en toda la ejecuciÛn del procedimiento
	...
	-- Eliminar la tabla temporal global despuÈs de usarla
	DROP TABLE ##Temp;
...
FROM OPENROWSET('Microsoft.ACE.OLEDB.12.0',
	...
	-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	--para solucionar error 7099, 7050 Win+R -->services.msc -->SQL Server (SQLEXPRESS)--> propiedades-->iniciar sesion-->cabiar a "Cuenta del sistema local", Marca la casilla "Permitir que el servicio interact˙e con el escritorio".
...
BEGIN
    ...
    PRINT 'SP Importar_Productos_importados ya existe --> se borrÛ';
END;
GO
...
BEGIN
    ...
    SELECT [NombreProducto], [Proveedor], [CategorÌa], [CantidadPorUnidad], [PrecioUnidad], ''ARS''
    FROM OPENROWSET(''Microsoft.ACE.OLEDB.12.0'',
        ''Excel 12.0;Database=' + @RutaArchivo + ';HDR=YES'',
        ''SELECT [NombreProducto], [Proveedor], [CategorÌa], [CantidadPorUnidad], [PrecioUnidad] FROM [Listado de Productos$]'')';
    ...
    PRINT 'RelaciÛn Proveedor-Producto insertada correctamente.';

    -- Eliminar la tabla temporal global despuÈs de la ejecuciÛn
    DROP TABLE ##Temp;
    PRINT '##Temp eliminada.';

    PRINT 'ImportaciÛn completada correctamente.';
END;
GO
...
BEGIN
     ...
     PRINT 'SP Importar_Catalogo ya existe --> se borrÛ';
END;
GO
...
>>>>>>> e6ec30cad21111a8505a70c3a2fc513356e70d9a
    ...
    -- Crear tabla temporal global para la clasificaciÛn
    ...
    -- Importar datos de clasificaciÛn desde Excel
    SET @SQL =' 
    INSERT INTO ##Temp_Clasificacion (linea_clasificacion, tipo_producto)
    SELECT [LÌnea de producto], [Producto]
    FROM OPENROWSET(''Microsoft.ACE.OLEDB.12.0'',
        ''Excel 12.0;Database=' + @RutaArchivoClasificacion + ';HDR=YES'',
        ''SELECT [LÌnea de producto], [Producto] FROM [Clasificacion productos$]'')';
...
BEGIN
    ...
    -- Insertar datos en la tabla final sin encriptaciÛn
...
BEGIN 
	...
	CREATE TABLE #Temp (
		...
		INSERT INTO @nombres VALUES ('Juan'), ('MarÌa'), ('Carlos'), ('Ana'), ('Pedro'), ('Laura'), ('Luis'), ('SofÌa');
		...
		SET 
			...
			fnac = DATEADD(DAY, -ABS(CHECKSUM(NEWID()) % 18250), GETDATE()); -- 50 aÒos aleatorios

		UPDATE #Temp SET	nombre_producto = REPLACE (nombre_producto, '√©', 'È');
		UPDATE #Temp SET	nombre_producto = REPLACE (nombre_producto, '√±', 'Ò');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√≥', 'Û');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√°', '·');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√∫', '˙');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '¬∫', '∫');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√ë', '—');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√≠', 'Ì'); 
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, 'Âçò', 'Ò');
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√É∫', '√∫'); 
		UPDATE #Temp SET 	nombre_producto = REPLACE (nombre_producto, '√Å', '¡');
